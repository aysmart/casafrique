...
<?php $query=mysqli_query($con,"SELECT * FROM blogpost WHERE Category='Blog Post' ORDER BY DateOfPost DESC LIMIT 0,3"); 
...
<div class="card-body">
<h4 class="card-title"><?php echo htmlentities($row10['BlogTittle']) ?></h4>
<p class="card-text"><?php echo substr($row10['ContentText'], 0,180)."..." ?></p>
...
<?php $query=mysqli_query($con,"SELECT * FROM blogpost WHERE Category='Blog Post' ORDER BY DateOfPost DESC LIMIT 3,3"); 
while($row10=mysqli_fetch_array($query)){
					?>
<div class="col-md-4" style="float:left">
<div class="card mb-2">
...
src="<?php echo $row10['PostBanner']?>" alt="Card image cap"></a>
<div class="card-body">
<h4 class="card-title" style="font-size:20px;"><?php echo htmlentities($row10['BlogTittle']) ?></h4>
<p class="card-text"><span class="dropcap1"><?php echo htmlentities(substr($row['ContentText'], 0,1)) ?></span><?php echo substr($row['PreviewText'], 0) ?></p>
<a class="btn btn-primary" href="blog-single?pid=<?php echo htmlentities($row10['ID']) ?>">Read More</a>
</div>
</div>
</div>
