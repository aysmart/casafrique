...
function twitterFeedWidget() {
	alert("I was here inside twitter costom.js");
	$(".twitter-feed").tweet({
		username: "casafrique",
...
function instagramFeedWidget() {
    var feed = new Instafeed({
	    get: 'user',
	    userId: 4904547688,  //Your ID Number Here - 1956899935 - 196729926 - 1555516340
...
function headerSubmitForm(){
	...
	var hfFName = $("#hfFName").val();
	var hfLName = $("#hfLName").val();
    var hfEmail = $("#hfEmail").val();
	var hfPhone = $("#hfPhone").val();
	var hfState = $("#hfState").val();
	var hfNation = $("#hfNation").val();
 
    $.ajax({
        type: "POST",
        url: "./php/hf-process.php",
        data: "hfFName=" + hfFName + "&hfLName=" + hfLName + "&hfEmail=" + hfEmail + "&hfPhone=" + hfPhone + "&hfState=" + hfState + "&hfNation=" + hfNation,
        success : function(text){
			if (text == "success"){
                headerFormSuccess();
            } else if (text == "registered"){
                headerFormRegistered();
            } else {
                headerFormError();
...
function headerFormRegistered(){
	$("#header-form")[0].reset();
	var successContent =  '<i class="hf-error-icon pe-7s-like2"></i>' +
	'<div class="hf-submit-msg-cont-inner">' +
		  '<h5>Error!!</h5>' +
		  '<p>You have registered before and you cannot register again.</p>';
	'</div>';	
	headerSubmitMSG(true, successContent);
}
...
function cfSubmitForm(){
    ...
    $.ajax({
        ...
        data: "cfName=" + name + "&cfEmail=" + email + "&cfSubject=" + subject + "&cfMessage=" + message,
        success : function(text){
            if (text == "success"){
                cfSuccess();
            } else {
...
$( ".subscribe-form" ).ajaxChimp({
    ...
    url: "https://sagencoders.us7.list-manage.com/subscribe/post?u=bf162b6194ee85d242eb2bed7&amp;id=95152bb0cd" //Replace this with your own mailchimp post URL. 
});
